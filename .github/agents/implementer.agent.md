...
**影響ファイル照合**: issue 本文の「影響ファイル」セクションに列挙されたファイルが、上記 `git diff` の出力に**すべて含まれているか**を1件ずつ確認する。漏れがある場合は実装に戻って対応する。

ビルド・テストが失敗した場合は修正してリトライする（最大 3 回）。3 回失敗した場合は draft PR を作成し、原因と試したことを本文に記載する。

### Step 5: PR 作成

- `main` ブランチへは直接 push しない
- タイトル: Conventional Commits 形式（`feat:` / `fix:` / `refactor:` / `test:`）
- 本文: 変更内容のサマリー + `Closes #N` でクローズするイシュー番号（**必須**）
- 本文末尾に影響ファイル照合結果を記載する:
  ```
  ## 影響ファイル照合
  - [x] internal/foo/bar.go
  - [x] internal/foo/baz.go
  ```
